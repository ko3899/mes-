(function () {
  ...
  async function login(event) {
    ...
    var result = await api.login({username: username, password: password}); button.disabled = false; button.textContent = '进入工作台';
    if (!result.ok) { error.textContent = result.message; return; } byId('passwordInput').value = ''; activate(result.data);
  }
