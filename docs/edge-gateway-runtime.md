...
HTTP模式用于内网试点和没有MQTT基础设施的客户；必须使用HTTPS。MQTT模式用于正式多工厂部署，固定使用QoS 1和双向TLS。
...
Broker必须校验客户证书并使用ACL限制网关只能发布自身主题。QoS 1 的PUBACK只代表Broker持久接收；中央MQTT消费者及业务入库属于下一中央Broker消费模块。在该模块启用前，生产数据闭环应继续使用已实现的HTTPS确认接口。
